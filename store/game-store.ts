...
function resetForNewHand(players: PlayerState[]) {
  return players.map((player) => {
    ...
    return {
      ...
      totalContribution: 0,
      cards: [],
      lastAction: shouldReset && player.isHero ? "Reset to $5,000. Welcome back, Mr. Bond." : undefined,
      readStats: {
        ...player.readStats,
...
function executeAction(
  ...
  if (livePlayers.length === 1) {
    ...
    const bonusXP = heroWon ? Math.floor(5 + Math.random() * 46) : 0;
    const xpEvents: XPEventId[] = [];
    if (state.handNumber === 1) {
      xpEvents.push("first_hand");
    }
    if (heroWon) {
      xpEvents.push("hand_win");
    }
  ...
  if (bettingClosed || everyoneAllInOrFolded) {
    ...
    if (nextPhase === "SHOWDOWN") {
      ...
      const handResult: HandHistoryEntry = {
        ...
        serverSeedHash: state.serverSeedHash,
        serverSeed: state.serverSeed,
        summary: `${winners.map((id) => players.find((player) => player.id === id)?.name ?? id).join(" & ")} won with ${heroEval?.label ?? "best hand"}.`,
      };
      return {
        players,
        communityCards: board,
